...
_redis.generate = function(prefix, len, callback, type) {
  if(!callback) {
    callback = len;
    len = 0;
  }
  len = len || 6
  var id = type == '32' ? genBase36Id(len) : genBase62Id(len);
  var key = prefix + ':' + id;
  var self = this;
  this.exists(key, function(err, exist) {
      if(err) return callback(err);
      if(exist) {
        process.nextTick(self.generate(prefix, len, callback));
      } else {
        callback(err, id, key);
      }
  })
...
_redis.generate32 = function(prefix, len, callback){
  this.generate(prefix, len, callback, '32');
}
