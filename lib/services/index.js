module.exports = function(redis) {
    ...
    function createUser(info, callback) {
        ...
        // username check
        if(!pattern_username.exec(username)) return callback(new Error('username is valid to createUser'));
        var keyUsernameToId = 'uname:' + username + ":id";
        redis.exists(keyUsernameToId, function(err, exists) {
    ...
    function getUser(uid, callback) {
        redis.hgetall(uid, function(err, info) {
    ...
    function getUserByName(uname, callback) {
        redis.get('uname:' + uname + ':id', function(err, id) {
                if(err) {return callback(err);}
                getUser(id, callback);
        })
    }
