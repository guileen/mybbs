...
exports.init = function(redisClient) {
    ...
    exports.createGroup = function createGroup(info, callback) {
        ...
        info.privacy = info.privacy || common.PRIVACY_TEAM;
        userdao.assertExistsUser(ownerId, function(err) {
                ...
                redis.generate32('g', 3, function(err, id, key) {
                        ...
                        redis.multi()
                          .hmset(key, info)
                          .sadd('all:groups:' + info.privacy, id)
...
exports.makeGroupRefCode = function(gid, uid, count, remark, callback) {
    exports.assertUserInGroup(uid, gid, function(err) {
        if(err) return callback(err);
        redis.generate('g.ref', 8, function(err, code, key) {
            redis.multi()
              .hmset(key, 'uid', uid, 'gid', gid, 'total', count, 'left', count, 'remark', remark|| '')
...
exports.getTopInviters = function(gid, max, callback) {
  redis.zrevrange('zinv:'+gid, 0, max, 'withscores', util.zsetStrip(function(err, uids, invites) {
        if(err) return callback(err);
        async.map(uids, userdao.get, function(err, users) {
            callback(err, users, invites);
        })
  }))
}
