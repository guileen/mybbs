...
exports.init = function(redisClient) {
    ...
    // g.inv:{gid}  someone invited how many users
...
exports.getAllRefCodeInfo = function(gid, uid, callback) {
  redis.zrevrange('zref:'+gid+':'+uid, 0, -1, 'withscores', util.zsetStrip(function(err, codes, scores) {
        async.map(codes, exports.getRefCodeInfo, function(err, results) {
            if(err) return callback(err);
...
exports.getRefCodeInfo = function(code, callback) {
    redis.hgetall('g.ref:' + code, function(err, info) {
        async.parallel([
            userdao.get.bind(null, info.uid)
          , exports.getGroup.bind(null, info.gid)
          ], function(err, results) {
            info.code = code
            info.user = results[0];
            info.group = results[1];
            callback(err, info);
        });
    });
}
