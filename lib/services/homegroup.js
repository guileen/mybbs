...
exports.onNewComment = function(tid, callback) {
  // TODO ignore inittopic
  redis.hget('t:'+tid, 'gid', function(err, gid) {
      if(err) callback(err);
      redis.zscore('g:' + gid + ':t', tid, function(err, tscore) {
          if(err) callback(err);
          var score = Math.round(tscore / 3 * 2 + Date.now()/3);
          redis.zadd('g:' + gid + ':t', score, tid, callback);
      });
  })
}
